...
/**
 * 200 OK — general success, with optional data, message, and meta.
 *
 * @param {import("express").Response} res
 * @param {*}      [data]    — response payload; omit for status-only actions
 * @param {string} [message] — optional human-readable message
 * @param {object} [meta]    — optional metadata block (pagination, provider info, etc.)
 */
function ok(res, data, message, meta) {
  if (meta !== undefined && meta !== null) {
    const body = { ok: true };
    if (message !== undefined && message !== null && message !== "") body.message = String(message);
