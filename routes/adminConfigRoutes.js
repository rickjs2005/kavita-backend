...
router.get("/", verifyAdmin, async (req, res, next) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT * FROM shop_settings WHERE id = ? LIMIT 1",
      [id]
    );
...
router.put("/", verifyAdmin, async (req, res, next) => {
  try {
    ...
    const boolOrCurrent = (val, curr) =>
      typeof val === "boolean" ? (val ? 1 : 0) : curr;

    const numOrCurrent = (val, curr) =>
      typeof val === "number" ? val : curr;
    ...
    const updateData = {
      ...
      checkout_require_cpf: boolOrCurrent(
        checkout_require_cpf,
        current.checkout_require_cpf
      ),
      checkout_require_address: boolOrCurrent(
        checkout_require_address,
        current.checkout_require_address
      ),
      checkout_allow_pickup: boolOrCurrent(
        checkout_allow_pickup,
        current.checkout_allow_pickup
      ),
      ...
      payment_pix_enabled: boolOrCurrent(
        payment_pix_enabled,
        current.payment_pix_enabled
      ),
      payment_card_enabled: boolOrCurrent(
        payment_card_enabled,
        current.payment_card_enabled
      ),
      payment_boleto_enabled: boolOrCurrent(
        payment_boleto_enabled,
        current.payment_boleto_enabled
      ),
      ...
      mp_sandbox_mode: boolOrCurrent(
        mp_sandbox_mode,
        current.mp_sandbox_mode
      ),

      shipping_flat_enabled: boolOrCurrent(
        shipping_flat_enabled,
        current.shipping_flat_enabled
      ),
      shipping_flat_value: numOrCurrent(
        shipping_flat_value,
        current.shipping_flat_value
      ),
      shipping_free_over: numOrCurrent(
        shipping_free_over,
        current.shipping_free_over
      ),
      shipping_region_text:
        shipping_region_text ?? current.shipping_region_text,
      shipping_deadline_text:
        shipping_deadline_text ?? current.shipping_deadline_text,

      comm_email_enabled: boolOrCurrent(
        comm_email_enabled,
        current.comm_email_enabled
      ),
      comm_whatsapp_enabled: boolOrCurrent(
        comm_whatsapp_enabled,
        current.comm_whatsapp_enabled
      ),
      ...
      facebook_pixel_id:
        facebook_pixel_id ?? current.facebook_pixel_id,
    };

    await db.query("UPDATE shop_settings SET ? WHERE id = ?", [
      updateData,
      id,
    ]);
