 ...
 *
 * Lê variáveis de ambiente, define utilitários puros e os três adapters de
 * storage (disk / S3 / GCS). Exporta o adapter selecionado para uso pelo
 * restante do módulo de mídia.
 *
 * Não tem dependência de nenhum outro módulo interno — pode ser carregado e
...
const createDiskAdapter = () => {
  // __dirname aqui é services/media/ — dois níveis acima da raiz do projeto
  ...
  return {
    ...
    resolveTargets: (inputs = []) =>
      normalizeTargets(inputs).map((target) => ({
        path: target.path,
    ...
    persist: async (files = [], options = {}) => {
      ...
      for (const file of files) {
        ...
        results.push({ path: publicPath, key: resolvedKey });
      }
