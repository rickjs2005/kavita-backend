...
async function listModelGallery(req, res) {
  try {
    ...
    return sendError(res, e instanceof AppError ? e : new AppError("Erro ao listar galeria.", 500, "SERVER_ERROR"));
  }
}
...
async function createModelGalleryItem(req, res) {
  ...
  try {
    ...
    if (!file) throw new AppError("Arquivo de mídia obrigatório.", 400, "VALIDATION_ERROR");

    const info = classify(file);
    if (!info) {
      safeUnlink(file);
      throw new AppError("Tipo de arquivo inválido. Use jpg/png/webp ou mp4.", 400, "VALIDATION_ERROR");
    }

    if (file.size > info.max) {
      safeUnlink(file);
      const mb = Math.round(info.max / 1024 / 1024);
      throw new AppError(`Arquivo excede ${mb}MB.`, 400, "VALIDATION_ERROR");
    }

    const saved = await mediaService.persistMedia([file], { folder: "drones" });
    const media_path = saved?.[0]?.path;
    if (!media_path) {
      throw new AppError("Falha ao salvar arquivo.", 500, "SERVER_ERROR");
    }
    ...
    return sendError(res, e instanceof AppError ? e : new AppError("Erro ao criar item.", 500, "SERVER_ERROR"));
  }
}
...
async function updateModelGalleryItem(req, res) {
  ...
  try {
    ...
    if (!itemId) throw new AppError("ID do item inválido.", 400, "VALIDATION_ERROR");
    ...
    if (file) {
      ...
      if (file.size > info.max) {
        safeUnlink(file);
        const mb = Math.round(info.max / 1024 / 1024);
        throw new AppError(`Excede ${mb}MB.`, 400, "VALIDATION_ERROR");
      }
      const saved = await mediaService.persistMedia([file], { folder: "drones" });
    ...
    if (file) safeUnlink(file);
    return sendError(res, e instanceof AppError ? e : new AppError("Erro ao atualizar item.", 500, "SERVER_ERROR"));
  }
}
...
async function deleteModelGalleryItem(req, res) {
  try {
    ...
    if (!affected) throw new AppError("Item não encontrado.", 404, "NOT_FOUND");

    return res.json({ message: "Item removido.", id: itemId });
  } catch (e) {
    console.error("[drones/admin] deleteModelGalleryItem error:", e);
    return sendError(res, e instanceof AppError ? e : new AppError("Erro ao remover item.", 500, "SERVER_ERROR"));
  }
}
...
async function listGallery(req, res) {
  try {
    ...
    return sendError(res, new AppError("Erro ao listar galeria.", 500, "SERVER_ERROR"));
  }
}
...
async function createGalleryItem(req, res) {
  ...
  try {
    ...
    if (!info) {
      safeUnlink(file);
      throw new AppError("Tipo inválido. Use jpg/png/webp ou mp4.", 400, "VALIDATION_ERROR");
    }
    ...
    const saved = await mediaService.persistMedia([file], { folder: "drones" });
    const media_path = saved?.[0]?.path;
    if (!media_path) throw new AppError("Falha ao salvar arquivo.", 500, "SERVER_ERROR");
...
async function updateGalleryItem(req, res) {
  ...
  try {
    ...
    if (!itemId) throw new AppError("ID inválido.", 400, "VALIDATION_ERROR");
