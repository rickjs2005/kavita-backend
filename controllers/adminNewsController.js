...
function validateStationCodeOrNull(v, field = "station_code") {
    ...
    if (!isA3 && !isNum) {
        return {
            error: { status: 400, code: "VALIDATION_ERROR", message: `${field} inválido (ex.: A827 ou 83692).`, details: { field } },
        };
    }
    if (sc.length > 10) {
        return { error: { status: 400, code: "VALIDATION_ERROR", message: `${field} inválido (máx 10).`, details: { field } } };
    }
    return { value: sc };
}
...
controller.listClima = async (req, res) => {
    try {
 ...
 */
controller.suggestClimaStations = async (req, res) => {
    try {
...
controller.createClima = async (req, res) => {
    try {
        const body = req.body || {};
...
controller.updateClima = async (req, res) => {
    try {
...
controller.deleteClima = async (req, res) => {
    try {
        const id = toInt(req.params.id, 0);
        if (!id) return fail(res, 400, "VALIDATION_ERROR", "ID inválido.");
...
async function fetchChuvaMmFromProvider(climaRow) {
    ...
    // Obs: timezone é obrigatório quando usa daily :contentReference[oaicite:2]{index=2}
    const now = new Date();
    const end = toYMD(now);
...
controller.syncClima = async (req, res) => {
    try {
        const id = toInt(req.params.id, 0);
        if (!id) return fail(res, 400, "VALIDATION_ERROR", "ID inválido.");
...
controller.listCotacoes = async (req, res) => {
    try {
...
controller.createCotacao = async (req, res) => {
    try {
        const body = req.body || {};
...
controller.updateCotacao = async (req, res) => {
    try {
        const id = toInt(req.params.id, 0);
        if (!id) return fail(res, 400, "VALIDATION_ERROR", "ID inválido.");
...
controller.deleteCotacao = async (req, res) => {
    try {
        const id = toInt(req.params.id, 0);
        if (!id) return fail(res, 400, "VALIDATION_ERROR", "ID inválido.");
...
controller.updatePost = async (req, res) => {
    try {
        ...
        for (const f of ["excerpt", "cover_image_url", "category", "tags", "status"]) {
            if (hasOwn(body, f)) patch[f] = body[f] ? String(body[f]).trim() : null;
        }
        ...
        if (hasOwn(body, "author_admin_id")) {
...
controller.publishPost = async (req, res) => {
    try {
        ...
        const patch = { status: "published", published_at: nowSql() };
        const result = await newsModel.updatePost(id, patch);
